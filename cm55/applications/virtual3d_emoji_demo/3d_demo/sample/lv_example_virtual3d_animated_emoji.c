...
#ifndef VIRTUAL3D_EMOJI_DEMO_CAMERA_DISTANCE
#define VIRTUAL3D_EMOJI_DEMO_CAMERA_DISTANCE 80
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_DRAG_DIV
#define VIRTUAL3D_EMOJI_DEMO_DRAG_DIV 1
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_DRAG_MAX
#define VIRTUAL3D_EMOJI_DEMO_DRAG_MAX 120
#endif

#define EMOJI_BASE_COLOR 0xffffffff //可根据需要进行修�?
...
static lx_action_cfg_item_t lx_action_cfg_table[] =
{
    ...
    {"thinking",
        2, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xff00f0f0, 0xffffffff, 0xffffffff},//思�?
    {"speaking",
    ...
    {"say_hello",
        6, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xff8f8fff, 0xff000000, EMOJI_BASE_COLOR},//打招�?
    {"thumbup",
        7, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xff8f8fff, 0xff000000, EMOJI_BASE_COLOR},//点赞
    {"sleepy",
        8, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xff8f8fff, 0xffffffff, 0xffffffff},//困�?
    {"anticipation",
    ...
    {"tap_earphonemode_normal",
        11, 1, 0, 100, 1, EMOJI_TAP_ICON_X, EMOJI_TAP_ICON_Y, 25, 65, 0x00000000, EMOJI_BASE_COLOR, 0xff000000, EMOJI_BASE_COLOR, 0xffffffff}, // 开关降�?正常
    {"tap_earphonemode_transparencymode",
        11, 1, 1, 100, 1, EMOJI_TAP_ICON_X, EMOJI_TAP_ICON_Y, 25, 65, 0x00000000, EMOJI_BASE_COLOR, 0xff000000, EMOJI_BASE_COLOR, 0xffffffff}, // 开关降�?通�?
    {"tap_earphonemode_denoise",
        11, 1, 2, 100, 1, EMOJI_TAP_ICON_X, EMOJI_TAP_ICON_Y, 25, 65, 0x00000000, EMOJI_BASE_COLOR, 0xff000000, EMOJI_BASE_COLOR, 0xffffffff}, // 开关降�?降噪
    {"tap_music_eq",
    ...
    {"tap_music_last",
        11, 1, 4, 100, 1, EMOJI_TAP_ICON_X, EMOJI_TAP_ICON_Y, 25, 65, 0x00000000, EMOJI_BASE_COLOR, 0xff000000, EMOJI_BASE_COLOR, 0xffffffff}, // 音乐控制-上一�?
    {"tap_music_mute",
    ...
    {"tap_music_next",
        11, 1, 6, 100, 1, EMOJI_TAP_ICON_X, EMOJI_TAP_ICON_Y, 25, 65, 0x00000000, EMOJI_BASE_COLOR, 0xff000000, EMOJI_BASE_COLOR, 0xffffffff}, // 音乐控制-下一�?
    {"tap_music_play",
    ...
    {"angry",
        13, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xff8f8fff, 0xffffffff, 0xffffffff},//愤�?
    {"poke",
    ...
    {"fear",
        17, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xffffb30f, 0xffffffff, 0xffffffff},//害�?
    {"cry",
    ...
    {"error_general_1/3",
        20, 2, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xffffffff, 0xffffffff, 0xffffffff},//状态异�?1
    {"error_general_2/3",
        20, 3, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xffffffff, 0xffffffff, 0xffffffff},//状态异�?2
    {"error_general_3/3",
        20, 4, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xffffffff, 0xffffffff, 0xffffffff},//状态异�?3
    {"press",
        21, 1, -1, -1, -1, 0, 0, 0, 0, 0x00000000, EMOJI_BASE_COLOR, 0xffffffff, 0xffffffff, 0xffffffff},//按住屏幕左右/上下�?
}; 
...
#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_X
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_X 0
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_Y
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_Y 0
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_DEADZONE
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_DEADZONE 4
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_RANGE_MAX
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_RANGE_MAX 64
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_RELEASE_DELAY_MS
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_RELEASE_DELAY_MS 40
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_PERIOD_MS
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_PERIOD_MS 16
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_DURATION_MS
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_DURATION_MS 420
#endif

#ifndef VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_EXTRA_MS
#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_EXTRA_MS 340
#endif

#define VIRTUAL3D_EMOJI_DEMO_FOLLOW_FP_SCALE 1024U

static bool s_follow_touching = false;
static uint32_t s_follow_angle = 0;
static uint32_t s_follow_range = 0;
static uint32_t s_follow_range_fp = 0;
static uint32_t s_follow_return_start_range_fp = 0;
static uint32_t s_follow_return_start_angle = 0;
static uint32_t s_follow_return_start_tick = 0;
static uint32_t s_follow_return_duration = VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_DURATION_MS;
static lv_timer_t *s_follow_hold_timer = NULL;
static lv_timer_t *s_follow_return_timer = NULL;
...
static int get_angle_from_center(int x, int y, int center_x, int center_y)
{
    double delta_x = x - center_x;
    double delta_y = y - center_y;
    double angle_rad = atan2(delta_x, -delta_y);
...
static void emoji_get_follow_center(int *center_x, int *center_y)
{
    lv_area_t area;

    if(emoji_virtual3d != NULL) {
        lv_obj_get_coords(emoji_virtual3d, &area);
        *center_x = ((int)area.x1 + (int)area.x2) / 2 + VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_X;
        *center_y = ((int)area.y1 + (int)area.y2) / 2 + VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_Y;
    }
    else {
        *center_x = LV_HOR_RES / 2 + VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_X;
        *center_y = LV_VER_RES / 2 + VIRTUAL3D_EMOJI_DEMO_FOLLOW_CENTER_OFFSET_Y;
    }
}

static uint32_t emoji_get_follow_angle(const lv_point_t *pt)
{
    int center_x;
    int center_y;

    emoji_get_follow_center(&center_x, &center_y);
    return (uint32_t)get_angle_from_center(pt->x, pt->y, center_x, center_y);
}

static uint32_t emoji_get_follow_range(const lv_point_t *pt)
{
    int center_x;
    int center_y;
    int dx;
    ...
    emoji_get_follow_center(&center_x, &center_y);
    dx = abs(pt->x - center_x);
    dy = abs(pt->y - center_y);
    distance = dx > dy ? dx : dy;

    if(distance <= VIRTUAL3D_EMOJI_DEMO_FOLLOW_DEADZONE) {
        return 0;
    }

    max_distance = LX_HOR_RES / 2;
    if(max_distance <= VIRTUAL3D_EMOJI_DEMO_FOLLOW_DEADZONE) {
        return VIRTUAL3D_EMOJI_DEMO_FOLLOW_RANGE_MAX;
    }

    distance -= VIRTUAL3D_EMOJI_DEMO_FOLLOW_DEADZONE;
    max_distance -= VIRTUAL3D_EMOJI_DEMO_FOLLOW_DEADZONE;
    if(distance > max_distance) {
        distance = max_distance;
    }

    return (uint32_t)((distance * distance * VIRTUAL3D_EMOJI_DEMO_FOLLOW_RANGE_MAX) / (max_distance * max_distance));
}

static void emoji_follow_update_from_point(const lv_point_t *pt, bool keep_center)
{
    uint32_t range = emoji_get_follow_range(pt);

    if(keep_center && range == 0 && s_follow_range > 0) {
        return;
    }

    s_follow_range = range;
    s_follow_range_fp = range * VIRTUAL3D_EMOJI_DEMO_FOLLOW_FP_SCALE;
    s_follow_angle = (range == 0) ? s_follow_angle : emoji_get_follow_angle(pt);
}

static void emoji_follow_apply(void)
{
    if(emoji_virtual3d == NULL || !s_follow_touching) {
        return;
    }

    lv_virtual3d_set_param2(emoji_virtual3d, "followRange", s_follow_range);
    lv_virtual3d_set_param2(emoji_virtual3d, "followAngle", s_follow_angle);
}

static void emoji_follow_reset(void)
{
    s_follow_touching = false;
    s_follow_angle = 0;
    s_follow_range = 0;
    s_follow_range_fp = 0;
    s_follow_return_start_range_fp = 0;
    s_follow_return_start_angle = 0;
    s_follow_return_start_tick = 0;
    s_follow_return_duration = VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_DURATION_MS;
...
static void emoji_follow_return_timer_cb(lv_timer_t *timer)
{
    uint32_t elapsed;
    uint32_t progress;
    uint32_t eased;
    ...
    if(emoji_virtual3d == NULL || s_follow_touching) {
        if(s_follow_return_timer != NULL) {
            lv_timer_pause(s_follow_return_timer);
        }
        return;
    }

    lv_timer_set_period(s_follow_return_timer, VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_PERIOD_MS);

    if(s_follow_return_start_tick == 0) {
        s_follow_return_start_tick = lv_tick_get();
    }

    elapsed = lv_tick_elaps(s_follow_return_start_tick);
    if(elapsed < s_follow_return_duration) {
        progress = (elapsed * 1024U) / s_follow_return_duration;
        eased = (3U * progress * progress - 2U * progress * progress * progress / 1024U) / 1024U;
        if(eased > 1024U) {
            eased = 1024U;
        }

        s_follow_range_fp = (s_follow_return_start_range_fp * (1024U - eased)) / 1024U;
        s_follow_range = (s_follow_range_fp + (VIRTUAL3D_EMOJI_DEMO_FOLLOW_FP_SCALE / 2U)) / VIRTUAL3D_EMOJI_DEMO_FOLLOW_FP_SCALE;
        s_follow_angle = s_follow_return_start_angle;
        lv_virtual3d_set_param2(emoji_virtual3d, "followRange", s_follow_range);
        lv_virtual3d_set_param2(emoji_virtual3d, "followAngle", s_follow_angle);
        return;
    }
...
static void emoji_follow_start_return(void)
{
    s_follow_touching = false;
    s_follow_return_start_range_fp = (s_follow_range_fp != 0) ?
                                     s_follow_range_fp :
                                     s_follow_range * VIRTUAL3D_EMOJI_DEMO_FOLLOW_FP_SCALE;
    s_follow_return_start_angle = s_follow_angle;
    s_follow_return_start_tick = 0;
    s_follow_return_duration = VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_DURATION_MS +
                               (s_follow_range * VIRTUAL3D_EMOJI_DEMO_FOLLOW_RETURN_EXTRA_MS) /
                               VIRTUAL3D_EMOJI_DEMO_FOLLOW_RANGE_MAX;

    if(s_follow_return_timer == NULL) {
        s_follow_return_timer = lv_timer_create(emoji_follow_return_timer_cb,
                                                VIRTUAL3D_EMOJI_DEMO_FOLLOW_RELEASE_DELAY_MS,
                                                NULL);
    }
    else {
        lv_timer_set_period(s_follow_return_timer, VIRTUAL3D_EMOJI_DEMO_FOLLOW_RELEASE_DELAY_MS);
        lv_timer_reset(s_follow_return_timer);
        lv_timer_resume(s_follow_return_timer);
    }
}

static void emoji_follow_hold_timer_cb(lv_timer_t *timer)
{
    LV_UNUSED(timer);
    emoji_follow_apply();
}
...
static void emoji_get_local_point(lv_point_t *pt)
{
    lv_area_t area;
    lv_coord_t width;
    lv_coord_t height;
    ...
    lv_obj_get_coords(emoji_virtual3d, &area);
    width = lv_obj_get_width(emoji_virtual3d);
    height = lv_obj_get_height(emoji_virtual3d);
    pt->x -= area.x1;
    pt->y -= area.y1;
...
static void touch_event_handler(lv_event_t *e) {
    ...
    if(code == LV_EVENT_PRESSED) {
        ...
        lv_indev_get_point(lv_indev_get_act(), &pt);
            ...
            s_follow_touching = true;
            emoji_follow_update_from_point(&pt, true);
        ...
        lv_indev_get_point(lv_indev_get_act(), &pt);
            ...
            offsetX = offsetX / VIRTUAL3D_EMOJI_DEMO_DRAG_DIV;
            offsetY = offsetY / VIRTUAL3D_EMOJI_DEMO_DRAG_DIV;
            if(offsetX > VIRTUAL3D_EMOJI_DEMO_DRAG_MAX) offsetX = VIRTUAL3D_EMOJI_DEMO_DRAG_MAX;
            if(offsetX < -VIRTUAL3D_EMOJI_DEMO_DRAG_MAX) offsetX = -VIRTUAL3D_EMOJI_DEMO_DRAG_MAX;
            if(offsetY > VIRTUAL3D_EMOJI_DEMO_DRAG_MAX) offsetY = VIRTUAL3D_EMOJI_DEMO_DRAG_MAX;
            if(offsetY < -VIRTUAL3D_EMOJI_DEMO_DRAG_MAX) offsetY = -VIRTUAL3D_EMOJI_DEMO_DRAG_MAX;

			//根据X和Y方向的位移差，判断是上下滑动，还是左右滑�?
			if(abs(offsetX) > abs(offsetY))
			{
				lv_virtual3d_set_param2(emoji_virtual3d, "offsetX", (uint32_t)offsetX);//左右滑动
                lv_virtual3d_set_param2(emoji_virtual3d, "offsetY", (uint32_t)0);
			}
			else
			{
                lv_virtual3d_set_param2(emoji_virtual3d, "offsetX", (uint32_t)0);
...
void qday_emoji_handle_touch_event(lv_event_t *e)
{
...
static uint32_t virtual3d_user_callback(uint16_t cmd, uint32_t param, uint32_t user_data)
{
    ...
    switch (cmd) {
        case LX_CMD_ID_INIT:
            ...
            lv_virtual3d_set_param2(obj,  "cameraDistance", (uint32_t)VIRTUAL3D_EMOJI_DEMO_CAMERA_DISTANCE);//设置视距，视距越小，画面越大，默认值：36

            lv_virtual3d_set_param2(obj, "followRange",  (uint32_t)0);//设置头部转向幅度�?~100，默认值：0，仅设置动作"animate_system_standby_smartring_follow"后有�?
	        lv_virtual3d_set_param2(obj, "followAngle", (uint32_t)0);//设置头部转向角度�?~360，默认值：0，仅设置动作"animate_system_standby_smartring_follow"后有�?
	
            lv_virtual3d_set_param2(obj, "offsetX", (uint32_t)0);//设置头部移动的X偏移，屏幕中心点�?，默认值：0，仅动作"animate_system_standby_touchscreen_press"有效
            lv_virtual3d_set_param2(obj, "offsetY", (uint32_t)0);//设置头部移动的Y偏移，屏幕中心点�?，默认值：0，仅动作"animate_system_standby_touchscreen_press"有效

            lv_virtual3d_set_param2(obj, "antialiasScale", (uint32_t)1200);//填充像素时放大画布，送显时缩小画布达到抗锯齿的效果，但是越大需要的内存越多，帧率降的越多，参数为倍数X1000，小于等�?000时有效，默认值：1073
        ...
        case LX_CMD_ID_DEINIT:
            ...
            emoji_follow_reset();
            if(s_follow_hold_timer != NULL)
            {
                lv_timer_del(s_follow_hold_timer);
                s_follow_hold_timer = NULL;
            }
            if(s_follow_return_timer != NULL)
            {
                lv_timer_del(s_follow_return_timer);
                s_follow_return_timer = NULL;
            }
...
lv_obj_t * lv_example_virtual3d_animated_emoji(lv_obj_t * p_container)
{
    ...
    emoji_virtual3d = lv_virtual3d_create(p_container,
                                            ...
                                            EMOJI_BG);
    lv_obj_align(emoji_virtual3d, LV_ALIGN_CENTER, VIRTUAL3D_EMOJI_DEMO_EMOJI_OFFSET_X, VIRTUAL3D_EMOJI_DEMO_EMOJI_OFFSET_Y);
    /* 设置回调函数 */
    ...
    if(s_follow_hold_timer == NULL)
    {
        s_follow_hold_timer = lv_timer_create(emoji_follow_hold_timer_cb, 50, NULL);
    }
